# 🤖 Super Genius Bot — VPS Edition (no Convex)

Versi standalone bot Polymarket esports **tanpa Convex**. Satu proses Node
(`pm2`) menjalankan semuanya: mesin trading, REST API, dan dashboard UI.
Database pakai SQLite bawaan Node (`node:sqlite`) — **nol dependency native**,
tinggal `npm install` + `npm run build` + `pm2 start`.
...
Aturan bot (sama persis dengan versi Freebuff):
...
Bisa lewat git atau scp. Folder yang dibutuhkan hanya `standalone/`:

```bash
# opsi A — git
git clone <repo-mu> /opt/genius-bot && cd /opt/genius-bot/standalone

# opsi B — scp dari laptop
scp -r standalone root@IP_VPS:/opt/genius-bot/
```

Jangan pernah jalanin dari folder yang berisi `.env` asli di tempat lain —
`.env` cuma boleh ada di dalam `standalone/`.
